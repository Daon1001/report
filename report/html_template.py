...
def _svg_bar_chart(data_dict, title="", width=600, height=200, colors=None):
    """SVG 막대그래프 생성. data_dict = {label: value, ...}"""
    ...
    bar_w = min(50, (width - 80) // len(items) - 10)
    chart_h = height - 50
    default_colors = ["#5B3A7A", "#7A4F9A", "#D4698A", "#FFB7C5", "#9B6FB0", "#E8A0BF"]
    if not colors:
        colors = default_colors
    
    svg = f'<svg viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:{width}px;height:auto;margin:8px 0;">'
    if title:
        svg += f'<text x="{width//2}" y="16" text-anchor="middle" font-size="12" font-weight="600" fill="#3D2B56">{title}</text>'
    ...
    for i, (label, val) in enumerate(items):
        ...
        bar_h = abs(val) / max_val * (chart_h - 30) if max_val > 0 else 0
        ...
        svg += f'<text x="{x + bar_w/2}" y="{y - 5}" text-anchor="middle" font-size="10" font-weight="600" fill="{color}">{val_text}</text>'
        # 라벨
        svg += f'<text x="{x + bar_w/2}" y="{base_y + 14}" text-anchor="middle" font-size="9" fill="#666">{label}</text>'
    
    # 기준선
    svg += f'<line x1="40" y1="{base_y}" x2="{width-10}" y2="{base_y}" stroke="#DDD" stroke-width="1"/>'
    svg += '</svg>'
    return svg
...
def _ratio_analysis_page(ratios: dict, years: list, year_labels: list) -> str:
    ...
    chart1 = _svg_bar_chart(stability_data, f"안정성 지표 ({lyl})", 400, 180, ["#5B3A7A","#D4698A","#9B6FB0"])
    chart2 = _svg_bar_chart(profit_data, f"수익성 지표 ({lyl})", 550, 180, ["#5B3A7A","#7A4F9A","#D4698A","#FFB7C5","#9B6FB0"])
...
def _ratio_analysis_page2(ratios: dict, years: list, year_labels: list) -> str:
    ...
    chart3 = _svg_bar_chart(growth_data, f"성장성 ({lyl})", 380, 180, ["#5B3A7A","#D4698A","#9B6FB0"])
    chart4 = _svg_bar_chart(activity_data, f"활동성 ({lyl})", 380, 180, ["#7A4F9A","#D4698A","#FFB7C5"])
...
def _expense_analysis_page(isc: dict, years: list, year_labels: list) -> str:
    ...
    exp_chart = _svg_bar_chart(exp_data, f"매출 대비 경비율 ({lyl})", 500, 180, ["#5B3A7A","#D4698A","#9B6FB0","#FFB7C5"])
...
.page-header { display: flex; align-items: center; gap: 16px; padding: 28px 60px; border-bottom: 2px solid #F0E6F3; }
.section-badge { background: linear-gradient(135deg, #5B3A7A, #7A4F9A); color: white; padding: 6px 16px; border-radius: 20px; font-size: 12px; font-weight: 700; letter-spacing: 1px; }
.page-title-main { font-size: 22px; font-weight: 800; color: #3D2B56; }
.page-body { padding: 24px 60px; }

/* Tables */
.data-table { width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 16px; }
.data-table th { background: linear-gradient(135deg, #5B3A7A, #7A4F9A); color: white; padding: 10px 12px; text-align: center; font-weight: 600; font-size: 12px; }
.data-table td { padding: 8px 12px; border-bottom: 1px solid #EDE4F0; text-align: center; }
...
def _retirement_plan_page(isc: dict, bs: dict, company: dict, years: list, year_labels: list, sim_params: dict = None) -> str:
    ...
    retire_pretax_chart = _svg_bar_chart({"1배": severance_1x, "2배": severance_2x, "3배": severance_3x}, "배수별 퇴직금 (세전, 만원)", 380, 170, ["#9B6FB0","#7A4F9A","#5B3A7A"])
    retire_aftertax_chart = _svg_bar_chart({"1배": severance_1x - tax_1x, "2배": severance_2x - tax_2x, "3배": severance_3x - tax_3x}, "배수별 퇴직금 (세후, 만원)", 380, 170, ["#D4698A","#B44B6A","#5B3A7A"])
...
def _salary_tax_simulation_page(isc: dict, company: dict, years: list, sim_params: dict = None) -> str:
    ...
    eff_chart = _svg_bar_chart(eff_chart_data, "실효세율 (%)", 580, 170, ["#5B3A7A","#6B4A8A","#7A4F9A","#8B65AA","#9B6FB0","#D4698A"])
    felt_chart = _svg_bar_chart(felt_chart_data, "체감 담세율 (법인세절감 반영, %)", 580, 170, ["#D4698A","#C45A7A","#B44B6A","#A43C5A","#943D5A","#5B3A7A"])
    ...
    # 비교 그래프
    compare_tax_chart = _svg_bar_chart({"① 급여": g_total, "② 배당": d_total, "③ 퇴직금": r_total}, "세금 합계 (만원)", 400, 180, ["#C62828","#E65100","#2E7D32"])
    compare_net_chart = _svg_bar_chart({"① 급여": g_net, "② 배당": d_net, "③ 퇴직금": r_net}, "실수령액 (만원)", 400, 180, ["#9B6FB0","#D4698A","#5B3A7A"])
...
<div class="page content-page">
    ...
    <div class="page-body">
        ...
        <div style="display:flex;gap:16px;margin-bottom:20px;">
            <div style="flex:1;text-align:center;padding:16px;background:#F8F9FC;border-radius:12px;border:1px solid #E5E7EB;">
                <div style="font-size:12px;color:#666;">세후 실수령액 (연)</div>
                <div style="font-size:28px;font-weight:800;color:#2E7D32;">{net_salary:,.0f}만원</div>
            </div>
            <div style="flex:1;text-align:center;padding:16px;background:#F8F9FC;border-radius:12px;border:1px solid #E5E7EB;">
                <div style="font-size:12px;color:#666;">실효세율</div>
                <div style="font-size:28px;font-weight:800;color:#C62828;">{eff_rate*100:.1f}%</div>
            </div>
            <div style="flex:1;text-align:center;padding:16px;background:#F8F9FC;border-radius:12px;border:1px solid #E5E7EB;">
                <div style="font-size:12px;color:#666;">체감 담세율 (법인세절감 반영)</div>
                <div style="font-size:28px;font-weight:800;color:#4A5FC1;">{felt_rate*100:.1f}%</div>
            </div>
        </div>
...
<div class="page content-page">
    ...
    <div class="page-body">
        ...
        <table class="data-table financial-table" style="margin-bottom:20px;">
