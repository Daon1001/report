...
def parse_company_overview_excel(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            # .xls 파일은 openpyxl이 못 읽으므로 xlsx로 변환 필요
            _wb = None
            _filepath = xls.io if hasattr(xls, 'io') else filepath
            
            try:
                _wb = _load_wb(_filepath)
            except Exception:
                # .xls → .xlsx 변환 시도
                import subprocess, tempfile, glob
                tmp_dir = tempfile.mkdtemp()
                subprocess.run(
                    ['libreoffice', '--headless', '--convert-to', 'xlsx', '--outdir', tmp_dir, _filepath],
                    capture_output=True, timeout=30
                )
                converted = glob.glob(os.path.join(tmp_dir, '*.xlsx'))
                if converted:
                    _wb = _load_wb(converted[0])
            
            if _wb:
                # 모든 시트에서 이미지 수집 (시트 순서 기반)
                all_images = []  # (sheet_index, image_index, bytes, width, height)
                for si, sname in enumerate(_wb.sheetnames):
                    _ws = _wb[sname]
                    for ii, img in enumerate(_ws._images):
                        try:
                            img_bytes = img._data()
                            img_w = img.width or 0
                            img_h = img.height or 0
                            all_images.append((si, ii, img_bytes, img_w, img_h, sname))
                        except:
                            pass
                
                # 이미지 분류 기준:
                # - 시트2(index 1): 기업프로필 페이지 → 신용등급 게이지, EW등급
                # - 마지막 시트 또는 마지막-1 시트: 재무진단 원형 그래프 5개
                # 
                # 전략: 시트별로 모은 후, 이미지 크기로 판별
                # - 신용등급 게이지: ~7KB (작은 반원)
                # - EW등급: ~12KB (컬러 반원)
                # - 재무진단: ~6KB (도넛 차트) × 5개
                # - 로고: ~45KB (반복)
                ...
                sheet_images = {}
                for si, ii, ib, iw, ih, sn in all_images:
                    sheet_images.setdefault(si, []).append((ii, ib, len(ib), iw, ih))
                
                # 시트 1 (index 1): 기업프로필 → 신용등급 + EW등급
                if 1 in sheet_images:
                    imgs = sheet_images[1]
                    # 로고(45KB급)와 큰 배경 제외, 나머지 중 작은 것부터 정렬
                    small_imgs = sorted(
                        [(ii, ib, sz) for ii, ib, sz, w, h in imgs if 3000 < sz < 20000],
                        key=lambda x: x[0]  # 원래 순서 유지
                    )
                    if len(small_imgs) >= 2:
                ...
                # 마지막 시트: 재무진단 (5개 도넛 차트, 5~8KB)
                last_si = len(_wb.sheetnames) - 1
                if last_si in sheet_images:
                    imgs = sheet_images[last_si]
                    # 도넛 차트 = 5~8KB 크기
                    donut_imgs = sorted(
                        [(ii, ib, sz) for ii, ib, sz, w, h in imgs if 3000 < sz < 15000],
                        key=lambda x: x[0]
                    )
