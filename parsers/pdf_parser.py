...
def parse_company_overview_excel(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 키-값 쌍 추출 (키 셀 옆이나 아래에 값이 있는 패턴)
        def find_value_after(keyword):
            for val, r, c, sheet, df in all_cells:
                if keyword in val and keyword != val:
                    # 같은 셀에 값이 포함된 경우
                    parts = val.replace(keyword, '').strip().strip(':：').strip()
                    if parts:
                        return parts
                if val == keyword or val.startswith(keyword):
                    # 오른쪽 셀들 탐색
                    for cc in range(c+1, min(c+8, df.shape[1])):
                        rv = df.iloc[r, cc]
                        if pd.notna(rv) and str(rv).strip():
                            return str(rv).strip()
            return ""
        
        data["기업명"] = find_value_after("기업명") or find_value_after("(주)")
        # (주)XXX 패턴 직접 찾기
        if not data["기업명"]:
        ...
        data["사업자번호"] = find_value_after("사업자번호")
