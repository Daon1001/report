...
def parse_company_overview_excel(filepath: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            _wb = _load_wb(xls.io if hasattr(xls, 'io') else filepath)
            images_b64 = {}
            
            for sname in _wb.sheetnames:
                _ws = _wb[sname]
                for i, img in enumerate(_ws._images):
                    img_bytes = img._data()
                    img_b64 = base64.b64encode(img_bytes).decode('utf-8')
                    key = f"{sname}_{i}"
                    images_b64[key] = img_b64
            
            # Page 2: image_1 = 신용등급 게이지, image_2 = EW등급
            if "Page 2_1" in images_b64:
                data["신용등급_이미지"] = images_b64["Page 2_1"]
            if "Page 2_2" in images_b64:
                data["EW등급_이미지"] = images_b64["Page 2_2"]
            
            # Page 6: image_1~5 = 재무진단 (성장성, 수익성, 재무구조, 부채상환능력, 활동성)
            diag_keys = ["성장성", "수익성", "재무구조", "부채상환능력", "활동성"]
            for idx, dk in enumerate(diag_keys):
                img_key = f"Page 6_{idx+1}"
                if img_key in images_b64:
                    data.setdefault("재무진단_이미지", {})[dk] = images_b64[img_key]
            
            # Page 4: image_1 = 기술력 그래프, image_2 = 요약재무상태표 차트
            if "Page 4_2" in images_b64:
                data["재무상태표_차트"] = images_b64["Page 4_2"]
            if "Page 5_1" in images_b64:
                data["손익계산서_차트"] = images_b64["Page 5_1"]
            if "Page 5_3" in images_b64:
                data["현금흐름_차트"] = images_b64["Page 5_3"]
