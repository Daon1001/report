...
if st.session_state.authenticated_user is None:
    ...
    st.markdown("<p style='color:#666; margin-bottom:30px;'>종합 경영진단 AI 마스터 v24.0 [Coordinate-Linker]</p>", unsafe_allow_html=True)
...
def extreme_parser(files):
    """PDF의 파편화된 텍스트 레이어를 강제로 재조합하여 추출"""
    res = {
    ...
    for file in files:
        if file.name.endswith('.pdf'):
            ...
            # KREtop/KODATA 특유의 엑셀형 텍스트 구조 대응 ("항목","값")
            # 1. 불필요한 따옴표와 쉼표 제거 및 정규화
            norm_txt = all_text.replace('"', '').replace(',', '')
            tight_txt = norm_txt.replace(" ", "").replace("\n", "").replace("\t", "")
            
            # 2. 키워드 기반 근접 탐색 (정규표현식 강화)
            # 기업명 추출: - 기업명 : (주)메이홈
            comp_match = re.search(r'기업명\s*[:：\- ]*([가-힣\(\)A-Za-z0-9&]+)', norm_txt)
            if comp_match: res['comp'] = comp_match.group(1).strip()
            
            # 대표자 추출: 대표자명 : 박승미
            ceo_match = re.search(r'대표자(?:명)?\s*[:：\- ]*([가-힣]{2,4})', norm_txt)
            if ceo_m := ceo_match: res['ceo'] = ceo_m.group(1).strip()
            
            # 종업원수 추출: 종업원수 10명
            emp_match = re.search(r'종업원수\s*[:：\- ]*(\d+)', norm_txt)
            if emp_m := emp_match: res['emp'] = int(emp_m.group(1))
            elif "종업원수" in tight_txt:
                e_val = re.search(r'종업원수(\d+)', tight_txt)
                if e_val: res['emp'] = int(e_val.group(1))
            
            # 3. 인증 현황 자동 인식
            cert_list = {'벤처': '벤처', '연구개발전담부서': '연구개발전담부서', '이노비즈': '이노비즈', '메인비즈': '메인비즈'}
            for key, kw in cert_list.items():
                if kw in tight_txt: res['certs'][key] = True
...
with col_l:
    ...
    if up_files:
        data = extreme_parser(up_files)
        st.success("✅ 파일 데이터 추출 성공 (Quantum-Linker 가동)")
...
with col_r:
    ...
    if up_files:
        ...
        st.info(f"분석 결과: 근로자 **{f_emp}명**으로 **'{labor_type} 사업장'** 노무 전용 가이드가 생성됩니다.")
        
        # 가치 평가 (단위 보정: 매출액이 10만 이하일 경우 백만원 단위로 판단)
        ...
        if st.button("🚀 종합 경영진단 보고서 발행 (재무제표 분석 포함)", type="primary", use_container_width=True):
            ...
            pdf.cell(190, 10, txt="▶ 전문가 재무 진단", ln=True)
            pdf.set_font("Nanum", size=11); pdf.set_text_color(0,0,0)
            debt_ratio = (r_debt / r_asset * 100) if r_asset > 0 else 0
            pdf.multi_cell(190, 8, txt=f"귀사의 2024년 부채비율은 {debt_ratio:.1f}%로 건전한 수준을 유지하고 있습니다. 매출액 성장세와 당기순이익의 흐름을 볼 때 향후 기업 가치가 더욱 상승할 것으로 예측됩니다.")
            ...
            fig.savefig("m_final_v24.png", dpi=300); pdf.image("m_final_v24.png", x=15, w=180)
            
            pdf.ln(5); pdf.set_font("Nanum", size=12); pdf.set_text_color(0,0,0)
            pdf.cell(190, 10, txt=f"■ 인증현황: 벤처({('보유' if c_vals.get('벤처') else '미보유')}), 전담부서({('보유' if c_vals.get('연구개발전담부서') else '미보유')})", ln=True)
