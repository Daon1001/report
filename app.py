...
if st.session_state.authenticated_user is None:
    ...
    st.markdown("<p style='color:#666; margin-bottom:30px;'>종합 재무진단 및 가치평가 시스템 v11.0</p>", unsafe_allow_html=True)
...
def smart_analyzer(files):
    """PDF의 텍스트 레이아웃을 정밀하게 분석하여 정보 추출"""
    res = {
    ...
    for file in files:
        ...
        if file.name.endswith('.pdf'):
            ...
            # 기업명 추출 로직 강화 (개요.pdf: - 기업명 : (주)메이홈)
            comp_match = re.search(r'기업명\s*[:|-]\s*([가-힣\(\)A-Za-z0-9]+)', full_text)
            if comp_match: res['comp'] = comp_match.group(1).strip()
            
            # 대표자 추출 로직 강화 (개요.pdf: - 대표자 : 박승미)
            ceo_match = re.search(r'대표자(?:명)?\s*[:|-]\s*([가-힣]{2,4})', full_text)
            if ceo_match: res['ceo'] = ceo_match.group(1).strip()
            
            # 종업원수 추출 로직 강화 (개요.pdf: 종업원수 10명)
            emp_match = re.search(r'종업원수\s*[:|-]?\s*(\d+)', full_text)
            if emp_match: res['emp'] = int(emp_match.group(1))
            
            # 인증 현황 (보유/미보유 키워드 매칭)
            for cert in res['certs'].keys():
                # "벤처 인증" 또는 "벤처인증" 등 다양한 형식 대응
                if f"{cert} 인증" in full_text or f"{cert}\s*인증" in full_text: 
                    res['certs'][cert] = True

        # 2. 엑셀/CSV 정밀 스캔 (1차년도 0값 누락 방지 및 천원 단위 환산)
...
# --- [4. 메인 화면 및 관리자 기능] ---
st.markdown('<div class="premium-header"><h1>📊 [PRIME] 종합 경영진단 및 가변형 리포트 시스템</h1></div>', unsafe_allow_html=True)
...
with col_left:
    ...
    if up_files:
        ...
        st.success("✅ 파일 데이터 추출 완료 (22~24년 매칭)")
        
        with st.expander("📝 추출 데이터 보정 및 최종 확인", expanded=True):
            # 파일에서 읽어온 데이터(메이홈, 박승미 등)로 자동 채움
...
with col_right:
    ...
    if up_files:
        ...
        st.info(f"현재 근로자 **{f_emp}명**으로 **'{labor_type} 사업장'** 법규 분석 페이지가 자동 생성됩니다.")
        
        # 가치 평가 (천원 -> 원 환산)
        stock_price = ((r_inc * 1000 / 0.1)*0.6 + (data['fin']['자산'][2]-data['fin']['부채'][2])*1000*0.4) / 100000
        ...
        if st.button("🚀 미다스형 종합 정밀 보고서 발행 (PDF)", type="primary", use_container_width=True):
            pdf = FPDF()
            ...
            cert_map = {
                ...
                "연구개발전담부서": "연구원 인건비의 25%를 세액공제 받을 수 있는 SME 최강의 절세 수단입니다.",
                "이노비즈": "기술력을 인정받아 금융권 금리 우대 및 입찰 가점 혜택이 있습니다."
            }
            for c, desc in cert_map.items():
                ...
                if "미보유" in status:
                    pdf.set_text_color(200, 0, 0); pdf.cell(190, 8, txt="→ 기업 경쟁력 향상을 위해 조속한 취득 전략 수립을 권장합니다.", ln=True)
            ...
            pdf.set_font("Nanum", size=12)
            pdf.cell(190, 10, txt=f"분석 결과: 현재 {f_emp}명으로 '{labor_type} 사업장' 기준이 적용됩니다.", ln=True)
            
            rules = [("연차 유급 휴가", "의무 (15일~)", "미적용 (자율)"), ("가산 수당(연장/야간)", "50% 가산 지급", "시급만 지급"), ("부당해고 구제", "노동위원회 신청 가능", "민사 소송만 가능")]
            ...
            st.download_button("💾 종합 정밀 보고서 다운로드", data=pdf_out, file_name=f"진단보고서_{f_comp}.pdf")
