...
if st.session_state.authenticated_user is None:
    ...
    st.markdown("<p style='color:#666; margin-bottom:30px;'>종합 경영진단 AI 마스터 v21.0 [Full-Spectrum]</p>", unsafe_allow_html=True)
...
def full_spectrum_analyzer(files):
    """문서의 모든 텍스트 파편을 재조합하여 데이터 강제 매칭"""
    res = {
...
with col_l:
    ...
    if up_files:
        data = full_spectrum_analyzer(up_files)
...
with col_r:
    st.subheader("📈 경영 지표 진단 및 미래 예측")
    if up_files:
        # 노무 타입 결정
        labor_type = "5인 이상" if f_emp >= 5 else "5인 미만"
        st.info(f"분석: 근로자 **{f_emp}명**으로 **'{labor_type} 사업장'** 전용 가이드가 생성됩니다.")
        ...
        if st.button("🚀 종합 경영진단 보고서 발행 (PDF)", type="primary", use_container_width=True):
            ...
            pdf.set_font("Nanum", size=15); pdf.set_text_color(11, 31, 82); pdf.cell(190, 15, txt=f"▶ 현시점 주당 가치: {int(stock_val):,} 원", ln=True)
            fig.savefig("m_rep.png", dpi=300); pdf.image("m_rep.png", x=15, w=180)
            ...
            cert_guide = {
                "벤처": "법인세 50% 감면 및 정부 정책자금 한도 우대 필수 인증",
