...
for key, val in [("logged_in", False), ("user_email", ""), ("user_name", ""), ("user_org", ""), ("user_title", ""), ("user_phone", "")]:
...
def show_login_page():
    ...
    with col:
        ...
        with st.form("login_form"):
            ...
            name = st.text_input("이름", placeholder="홍길동 (최초 1회만 입력)")
            org = st.text_input("소속", placeholder="중소기업경영지원단 (최초 1회만 입력)")
        ...
        if submitted and email:
            ...
            if status == "approved":
                ...
                st.session_state.user_name = profile.get("name") or name or email
                st.session_state.user_org = profile.get("org") or org or ""
...
def _report_ui():
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            for f in uploaded_files:
                ...
                if ftype in ("overview_pdf", "overview_xls"):
                    file_map["overview"] = f
    ...
    if has_required:
        ...
        try:
            ...
            st.caption("필요한 챕터만 선택해서 맞춤형 리포트를 생성할 수 있습니다 · 모두 선택 시 전체 리포트(약 49페이지) 생성")
            ...
            with ch_cols1[2]:
                ch_valuation = st.checkbox("📈 **기업가치평가**", value=True,
                    help="비상장주식 가치·상속세 시뮬레이션 (약 4페이지)")
            
            with ch_cols2[0]:
                ch_executive = st.checkbox("👔 **임원소득보상플랜**", value=True,
                    help="급여·배당·퇴직금 시뮬레이션 (약 3페이지)")
            with ch_cols2[1]:
                ch_dividend = st.checkbox("💰 **배당플랜**", value=True,
                    help="배당전략·미처분이익잉여금·종신보험 (약 4페이지)")
            with ch_cols2[2]:
                ch_governance = st.checkbox("📋 **기업제도정비**", value=True,
                    help="정관·법인관리·인증·특허전략 (약 6페이지)")
            ...
            selected_pages = {
                "finance": ch_finance, "credit": ch_credit, "valuation": ch_valuation,
                "executive": ch_executive, "dividend": ch_dividend, "governance": ch_governance,
            ...
            if st.button("🚀 리포트 생성", type="primary", use_container_width=True, disabled=(n_selected == 0)):
                with st.spinner("PDF 생성 중..."):
                    ...
                    html = generate_report_html(company=company, bs=bs, isc=isc, mfg=mfg,
                        ...
                        sim_params=sim_params, selected_pages=selected_pages)
