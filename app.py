...
if st.session_state.authenticated_user is None:
    ...
    st.markdown("<p>재무경영진단 AI 마스터 v4.0 (종합 보고서 모드)</p>", unsafe_allow_html=True)
    ...
    if col_r.button("승인 신청", use_container_width=True):
        if login_email and user_db[user_db['email'] == login_email].empty:
            ...
            st.info("이미 등록되어 있거나 신청된 이메일입니다.")
            
    st.markdown('</div>', unsafe_allow_html=True)
    st.stop()

# --- [3. 정밀 데이터 추출 엔진 (PDF & Excel 천원단위 대응)] ---
...
def smart_excel_parser(file):
    ...
    res = {
        ...
        '자산총계': [0, 0, 0], '부채총계': [0, 0, 0], '원재료비': [0, 0, 0], '판관비': [0, 0, 0]
    }
    try:
            ...
            for _, row in df.iterrows():
                ...
                for key in res.keys():
                    if key in row_txt:
                        ...
                        if len(nums) >= 3: res[key] = nums[-3:] # 최근 3개년
    except: pass
    return res

# --- [4. 메인 화면 및 관리자 기능] ---

st.markdown('<div class="premium-header"><h1>📊 종합 재무진단 및 비상장주식 가치평가</h1></div>', unsafe_allow_html=True)
    ...
    if up_files:
        ...
        if e_res:
            ...
            with st.expander("📝 추출 데이터 보정 및 입력", expanded=True):
                ...
                # 천원 단위 데이터를 원 단위로 정확히 계산
                revs = [st.number_input(f"{i+1}차 매출(천원)", value=float(e_res['매출액'][i])) for i in range(3)]
                incs = [st.number_input(f"{i+1}차 순이익(천원)", value=float(e_res['당기순이익'][i])) for i in range(3)]
                mats = [st.number_input(f"{i+1}차 원재료(천원)", value=float(e_res['원재료비'][i])) for i in range(3)]
                exps = [st.number_input(f"{i+1}차 경비(천원)", value=float(e_res['판관비'][i])) for i in range(3)]
                assets = [st.number_input(f"{i+1}차 자산(천원)", value=float(e_res['자산총계'][i])) for i in range(3)]
                debts = [st.number_input(f"{i+1}차 부채(천원)", value=float(e_res['부채총계'][i])) for i in range(3)]
...
with col2:
    ...
    if e_res:
        ...
        stock_price = ((w_inc_won / 0.1)*0.6 + (assets[2]-debts[2])*1000*0.4) / total_shares
        ...
        if st.button("🚀 미다스형 종합 리포트 발행 (PDF)", type="primary", use_container_width=True):
            ...
            # Page 2: 재무 현황
            pdf.add_page(); pdf.set_text_color(0,0,0); pdf.set_font("Nanum", size=16)
            pdf.cell(190, 15, txt="1. 3개년 주요 재무 지표 (단위: 천원)", ln=True); pdf.line(10, 25, 200, 25); pdf.ln(10)
            pdf.set_font("Nanum", size=12)
            pdf.cell(190, 10, txt=f"■ 최신 매출액: {revs[2]:,.0f} | 순이익: {incs[2]:,.0f}", ln=True)
            pdf.cell(190, 10, txt=f"■ 부채비율: {(debts[2]/assets[2]*100):.1f}% | 원재료비율: {(mats[2]/revs[2]*100):.1f}%", ln=True)
            
            # Page 3: 가치 평가
            pdf.add_page(); pdf.set_font("Nanum", size=16); pdf.cell(190, 15, txt="2. 비상장주식 가치평가 및 미래 예측", ln=True)
            pdf.line(10, 25, 200, 25); pdf.ln(10); pdf.set_font("Nanum", size=15); pdf.set_text_color(11, 31, 82)
            pdf.cell(190, 15, txt=f"▶ 현재 주당 가치: {int(stock_price):,}원", ln=True)
            fig.savefig("f_rep.png", dpi=300); pdf.image("f_rep.png", x=15, w=180)

            # Page 4: 컨설팅 솔루션
            pdf.add_page(); pdf.set_text_color(0,0,0); pdf.set_font("Nanum", size=16)
            pdf.cell(190, 15, txt="3. 전문가 경영 컨설팅 제안", ln=True); pdf.line(10, 25, 200, 25); pdf.ln(10)
            pdf.set_font("Nanum", size=11)
            pdf.multi_cell(180, 8, txt="[원가관리] 원재료 비중이 상승세에 있어 구매처 다변화가 필요합니다.\n[가업승계] 미래 가치가 급격히 상승 중입니다. 증여세 부담을 줄이기 위한 사전 지분 증여가 권장됩니다.")
