    ...
    with st.sidebar:
        ...
        def detect_file_type(f):
            ...
            if name.endswith('.pdf'):
                ...
                try:
                    import tempfile
                    from parsers.pdf_parser import is_tax_adjustment_pdf
                    with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp:
                        tmp.write(f.getvalue())
                        tmp_path = tmp.name
                    is_tax_doc = is_tax_adjustment_pdf(tmp_path)
                    os.unlink(tmp_path)
                    if is_tax_doc:
        ...
        if uploaded_files:
            ...
            if file_map["tax_adjustment"]:
                st.success(f"🎯 **세무조정계산서 인식**: `{file_map['tax_adjustment'].name}`")
                st.caption("✨ 이 PDF 1개로 재무상태표·손익계산서·기업정보가 모두 자동 추출됩니다!")
            else:
                labels = {
        ...
        try:
            with st.spinner("📊 분석 중..."):
                if has_tax_doc:
                    ...
                    from parsers.pdf_parser import parse_tax_adjustment_pdf, extract_tax_deep_analysis
            ...
            chapter_count_caption = "약 51페이지" if has_tax_doc else "약 49페이지"
            ...
            with ch_cols1[1]:
                ...
                ch_tax_deep = st.checkbox("💼 **세무 심층진단** ⭐NEW", 
                    value=has_tax_doc, 
                    disabled=not has_tax_doc,
                    help="법인세 조정 결과·절세 효율·손금불산입 리스크 분석 (약 2페이지) · 세무조정계산서 업로드 시 활성화")
            with ch_cols1[2]:
            ...
            selected_pages = {
                ...
                "credit": ch_credit, "valuation": ch_valuation,
