...
def _report_ui():
    ...
    if has_required:
        ...
        try:
            ...
            # 자동 판별 결과 안내 + 수동 강제 옵션
            with st.expander("🔧 사업자 유형 (자동 판별됨 · 필요시 수동 변경)", expanded=False):
                col_a, col_b = st.columns([1, 2])
                with col_a:
                    detect_msg = "👤 개인사업자" if auto_is_personal else "🏢 법인"
                    st.markdown(f"**자동 판별 결과:** {detect_msg}")
                with col_b:
                    force_personal = st.checkbox(
                        "✅ 개인사업자로 강제 처리",
                        value=auto_is_personal,
                        help="자동 판별이 잘못된 경우 체크/해제로 수동 변경 가능. 개인사업자면 비상장주식·임원·배당·정관 챕터가 자동 OFF됩니다."
                    )
            
            # 최종 결정값
            is_personal = force_personal
