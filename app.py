...
if st.session_state.auth_user is None:
    st.markdown('<div style="background:white; padding:40px; border-radius:15px; max-width:450px; margin:10vh auto; text-align:center; border-top:8px solid #0b1f52; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">', unsafe_allow_html=True)
    st.markdown('<h2 style="color:#0b1f52;">🏛️ 중소기업경영지원단</h2>', unsafe_allow_html=True)
    st.markdown("<p style='color:#666;'>종합 재무진단 시스템 v28.0 [Cloud-Master]</p>", unsafe_allow_html=True)
...
# --- [2. 초정밀 좌표 기반 스캐너 (Spatial Analysis)] ---
...
def cloud_spatial_parser(files):
    """pdfplumber를 통한 표 구조 및 좌표 정밀 매칭"""
    res = {
        ...
        if file.name.endswith('.pdf'):
            with pdfplumber.open(file) as pdf:
                ...
                for page in pdf.pages:
                    ...
                    # (주)메이홈, 박승미 데이터가 있는 표 구조 분석
                    tables = page.extract_tables()
                    for table in tables:
                        for row in table:
                            row_str = " ".join([str(c) for c in row if c])
                            if '기업명' in row_str: res['comp'] = str(row[-1]).split('\n')[0].strip()
                            if '대표자' in row_str: res['ceo'] = str(row[-1]).split('\n')[0].strip()
                            if '종업원수' in row_str: res['emp'] = int(clean_num(row[-1]))
...
with st.sidebar:
    st.write(f"👤 접속자: **{st.session_state.auth_user}**")
...
with col_l:
    ...
    up_files = st.file_uploader("개요.pdf 및 재무 엑셀(3개년) 업로드", accept_multiple_files=True)
    if up_files:
        data = cloud_spatial_parser(up_files)
        st.success("✅ [Spatial-Scan] 좌표 및 표 구조 인식 완료")
        with st.expander("📝 데이터 최종 확인 및 보정", expanded=True):
            ...
            r_rev = st.number_input("2024 매출액 (천원/백만원)", value=data['fin']['매출'][2])
...
with col_r:
    st.subheader("📈 실시간 리포트 구성")
    if up_files:
        labor_type = "5인 이상" if f_emp >= 5 else "5인 미만"
        st.info(f"분석: 현재 근로자 **{f_emp}명**으로 **'{labor_type} 사업장'** 전용 가이드가 생성됩니다.")
        ...
        ax.set_title(f"{f_comp} 주식 가치 상승 시뮬레이션")
        st.pyplot(fig)
        ...
        if st.button("🚀 종합 리포트 발행 (재무제표 분석 포함)", type="primary", use_container_width=True):
            ...
            pdf.cell(190, 15, txt="1. 정밀 재무제표 분석 (단위: 천원)", ln=True); pdf.line(10, 28, 200, 28); pdf.ln(15)
            ...
            f_rows = [("자산 총계", data['fin']['자산'][1], r_asset), ("부채 총계", data['fin']['부채'][1], r_debt), ("매출액", data['fin']['매출'][1], r_rev), ("당기순이익", data['fin']['이익'][1], r_inc)]
            ...
            pdf.cell(190, 10, txt="▶ 전문가 재무 건전성 진단", ln=True)
            pdf.set_font("Nanum", size=11); pdf.set_text_color(0,0,0)
            d_ratio = (r_debt / r_asset * 100) if r_asset > 0 else 0
            pdf.multi_cell(190, 8, txt=f"분석 결과, {f_comp}의 부채비율은 {d_ratio:.1f}%로 양호한 수준입니다. 매출액이 전년 대비 큰 폭으로 증가하고 있어 성장이 뚜렷하며, 향후 수익성 중심의 관리가 필요합니다.")

            # P3: 기업가치 및 인증/노무
            pdf.add_page(); pdf.set_font("Nanum", size=20)
            pdf.cell(190, 15, txt="2. 주식가치 평가 및 로드맵", ln=True); pdf.line(10, 28, 200, 28); pdf.ln(15)
            fig.savefig("v28_chart.png", dpi=300); pdf.image("v28_chart.png", x=15, w=180)
            ...
            pdf.cell(190, 10, txt=f"■ 노무: 근로자 {f_emp}명에 따른 '{labor_type} 사업장' 법적 기준 적용", ln=True)
