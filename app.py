...
if st.session_state.authenticated_user is None:
    ...
    st.markdown("<p style='color:#666; margin-bottom:30px;'>종합 경영진단 AI 마스터 v16.0 [Final Master]</p>", unsafe_allow_html=True)
...
def smart_analyzer(files):
    """Deep Core Scan: 텍스트 파편화 방지 및 위치 정보 기반 정밀 추출"""
    res = {
    ...
    for file in files:
        if file.name.endswith('.pdf'):
            ...
            # 텍스트 노멀라이징: 모든 종류의 공백 및 특수문자 대응
            norm_txt = re.sub(r'\s+', ' ', txt)
            tight_txt = txt.replace(" ", "").replace("\n", "").replace("\t", "").replace(":", "")
            
            # 1. 기업명 추출 (위치 기반 정밀 매칭: (주)메이홈)
            comp_m = re.search(r'기업명\s*[:：\s-]*\s*([가-힣\(\)A-Za-z0-9]+)', norm_txt)
            if comp_m: res['comp'] = comp_m.group(1).strip()
            
            # 2. 대표자 추출 (박승미)
            ceo_m = re.search(r'대표자(?:명)?\s*[:：\s-]*\s*([가-힣]{2,4})', norm_txt)
            if ceo_m: res['ceo'] = ceo_m.group(1).strip()
            
            # 3. 종업원수 추출 (10명)
            emp_m = re.search(r'종업원수\s*[:：\s-]*\s*(\d+)', norm_txt)
...
st.markdown('<div class="premium-header"><h1>📊 [MASTER] 파일 통합 경영진단 시스템</h1></div>', unsafe_allow_html=True)
...
with col_l:
    ...
    if up_files:
        ...
        st.success("✅ 파일 데이터 추출 성공 (Deep Core Scan 적용)")
        
        with st.expander("📝 데이터 보정 및 최종 확인", expanded=True):
            # 추출된 정보(메이홈, 박승미, 10명)를 자동으로 채움
...
with col_r:
    ...
    if up_files:
        ...
        if st.button("🚀 종합 경영진단 보고서 발행 (PDF)", type="primary", use_container_width=True):
            ...
            fig.savefig("v_master_chart.png", dpi=300); pdf.image("v_master_chart.png", x=15, w=180)
            
            # --- [PAGE 3: 기업 인증 진단 (보유 여부 연동)] ---
            pdf.add_page(); pdf.set_text_color(0,0,0); pdf.set_font("Nanum", size=20)
            ...
            for c, desc in cert_guide.items():
                ...
                if "미보유" in status:
                    pdf.set_text_color(200, 0, 0); pdf.cell(190, 8, txt="→ 기업 경쟁력 강화를 위한 조속한 취득을 제안합니다.", ln=True)
                pdf.ln(5); pdf.set_text_color(0,0,0)

            # --- [PAGE 4: 노무 기준법 가이드 (인원수 연동)] ---
