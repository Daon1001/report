...
if st.session_state.authenticated_user is None:
    ...
    st.markdown("<p style='color:#666; margin-bottom:30px;'>종합 재무진단 및 가치평가 시스템 v9.0</p>", unsafe_allow_html=True)
    ...
    if col_login.button("로그인", type="primary", use_container_width=True):
        ...
        if not user_row.empty and user_row.iloc[0]['approved']:
            ...
            st.warning("⚠️ 승인 대기 중입니다. 관리자에게 문의하세요.")
        else:
...
# --- [3. 정밀 데이터 추출 엔진 (1차년도 고정 및 특수기호 대응)] ---
    ...
    for file in files:
        try:
            ...
            for _, row in df.iterrows():
                ...
                full_row_txt = "".join(row_list)
                for key, keywords in mapping.items():
                    if any(kw in full_row_txt for kw in keywords):
                        # 엑셀 구조상 B, C, D 열(인덱스 2, 3, 4)이 2022, 2023, 2024년 데이터임
                        try:
                            v1 = clean_financial_value(row.values[2])
                            v2 = clean_financial_value(row.values[3])
                            v3 = clean_financial_value(row.values[4])
...
with col_left:
    ...
    if up_files:
        p_info = None
        for f in up_files:
            if f.name.endswith('.pdf'): p_info = extract_pdf_meta(f)
        
        extracted_res = robust_excel_parser(up_files)
        
        if any(v != [0.0, 0.0, 0.0] for v in extracted_res.values()):
            st.success("✅ 1차(22년)~3차(24년) 데이터 정밀 연동 완료")
            with st.expander("📝 데이터 보정 및 최종 확인 (단위: 천원)", expanded=True):
                # 허자현 대표님의 성함 및 기업명 자동 연동
                c_name = st.text_input("🏢 기업명", p_info['company'] if p_info else "신용")
                c_ceo = st.text_input("👤 대표자", p_info['ceo'] if p_info else "허자현")
                
                revs = [st.number_input(f"{2022+i}년 매출액(천원)", value=extracted_res['매출액'][i]) for i in range(3)]
                incs = [st.number_input(f"{2022+i}년 순이익(천원)", value=extracted_res['당기순이익'][i]) for i in range(3)]
                assets = [st.number_input(f"{2022+i}년 자산총계(천원)", value=extracted_res['자산총계'][i]) for i in range(3)]
                debts = [st.number_input(f"{2022+i}년 부채총계(천원)", value=extracted_res['부채총계'][i]) for i in range(3)]
                mats = [st.number_input(f"{2022+i}년 원재료비(천원)", value=extracted_res['원재료비'][i]) for i in range(3)]
                shares = st.number_input("발행주식 총수", value=100000)
                
                final_report_data = {"name": c_name, "ceo": c_ceo, "revs": revs, "incs": incs, "assets": assets, "debts": debts, "mats": mats, "shares": shares}

with col_right:
    st.subheader("📈 경영 지표 진단 및 가치 시뮬레이션")
    if final_report_data:
        ...
        # 성장률 계산 (2022 -> 2024)
        growth = (revs[2]/revs[0])**(1/2) - 1 if revs[0]>0 else 0.05
        ...
        if st.button("🚀 미다스형 종합 리포트 발행 (PDF)", type="primary", use_container_width=True):
            ...
            pdf.cell(190, 15, txt="1. 3개년 주요 재무 현황 (단위: 천원)", ln=True)
            ...
            pdf.cell(190, 10, txt=f"■ 매출액(22~24): {revs[0]:,.0f} -> {revs[1]:,.0f} -> {revs[2]:,.0f}", ln=True)
            ...
            pdf.cell(190, 15, txt="2. 비상장주식 평가 및 미래 시뮬레이션", ln=True)
            pdf.line(10, 28, 200, 28); pdf.ln(10); pdf.set_font("Nanum", size=15); pdf.set_text_color(11, 31, 82)
            pdf.cell(190, 15, txt=f"▶ 현시점 주당 가액: {int(stock_price):,}원", ln=True)
            fig.savefig("midas_final_report.png", dpi=300, bbox_inches='tight')
            pdf.image("midas_final_report.png", x=15, w=180)

            # --- Page 4: 전략적 컨설팅 제안 ---
            pdf.add_page(); pdf.set_text_color(0,0,0); pdf.set_font("Nanum", size=18)
            pdf.cell(190, 15, txt="3. 전문가 경영 리스크 진단", ln=True)
            pdf.line(10, 28, 200, 28); pdf.ln(15); pdf.set_font("Nanum", size=12)
            pdf.multi_cell(180, 10, txt="[진단 결과] 현재 기업 가치가 가파르게 상승하고 있습니다. 가업 승계 시 세부담을 줄이기 위한 사전 지분 구조 정비가 시급하며, 원재료비 비중 상승에 따른 원가 방어 전략이 필요합니다.")
