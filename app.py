...
def _report_ui():
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            for f in uploaded_files:
                ...
                if name.endswith(('.xlsx', '.xls')):
                    # CRETOP report.xls (기업개요/신용등급)
                    if name.startswith('report'):
                        if '__1_' in name or '__1__' in name or '(1)' in name:
                            file_map["credit"] = f     # report__1_.xls = 신용등급
                        elif '__' not in name and '(' not in name:
                            file_map["overview"] = f   # report.xls = 기업개요
                    # ETFI112E1 시리즈 (재무제표)
                    elif '__1_' in name or '__1__' in name or '(1)' in name:
                        file_map["is"] = f        # 손익계산서
                    ...
                    if any(kw in name for kw in ['개요', 'overview', '브리핑', '기업개요', '기업브리핑']):
                        file_map["overview"] = f
                    elif any(kw in name for kw in ['신용', 'credit', '등급', '신용등급']):
                        file_map["credit"] = f
        ...
        st.markdown("---")
        st.header("🏢 기업 정보")
        st.caption("PDF에서 자동 추출되지 않으면 직접 입력하세요")
    ...
    def merge_manual_input(company_data):
        ...
        for key, val in manual_fields.items():
            if val and val.strip():  # 수동 입력값이 있으면 덮어쓰기
